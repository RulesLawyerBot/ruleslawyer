...
public class SlackHelpService {
    ...
    public static final String SLACK_MAIN_HELP = "Welcome to RulesLawyer: Slack edition! This is a closed alpha. Please give feedback to Elaine Cao / Oritart.\n" +
            ...
            "\n" +
            "If a query results in a \"timeout\" error, try re-sending the query after a few seconds. For now the application is running on a free Heroku dyno, which will be upgraded once we're actually ready for public use so that will disappear. Any other error messages or strange output should be reported, along with what query caused the strange message. This uses the same backend as ruleslawyer-discord, but differing display requirements mean that the output will be slightly different.\n" +
            "\n" +
